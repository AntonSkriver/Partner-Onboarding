    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-3">
          ...
          <div>
            <h1 className="text-3xl font-semibold text-gray-900">{tn('myProjects')}</h1>
          </div>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <Button className="bg-purple-600 text-white hover:bg-purple-700">{t('newProject')}</Button>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as typeof activeTab)} className="space-y-6">
        <TabsList>
          <TabsTrigger value="in-progress">
            {t('inProgress', { count: inProgressProjects.length })}
          </TabsTrigger>
          <TabsTrigger value="finished">
            {t('finished', { count: finishedProjects.length })}
          </TabsTrigger>
        </TabsList>
...
function ProjectCard({ item, membershipIds }: { item: DecoratedProject; membershipIds: Set<string> }) {
  ...
  const teacherName = isHost ? t('youHost') : fullTeacherName
      ...
      <CardContent className="flex flex-1 flex-col space-y-4 p-5">
          ...
          <div className="flex items-center gap-2 flex-wrap min-w-0">
            ...
            {roleLabel ? (
              ...
              <Badge variant="secondary" className="bg-purple-50 text-purple-700 hover:bg-purple-100 gap-1.5 px-2 py-1 font-medium rounded-md">
                ...
                {program?.program.isPublic ? tc('public') : tc('private')}
              </Badge>
            )}
          </div>
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-500">
            {t('hostedBy')} <span className={cn("font-medium", isHost ? "text-gray-900" : "text-gray-900")}>{teacherName}</span>
          </p>
        </div>
